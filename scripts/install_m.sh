    ...
    then
        ...
        then
            ...
            then
                ...
                echo "The package $1 is not available in $comp_folder and must be installed manually"
            fi
        else
            echo "The folder $comp_folder not found: install $1 manually"
        fi
    fi
}
...
then
    ...
    read -p "Do you wanna install firos to communicate with fiware world ([Y]es/no)? " inst_firos
    case ${inst_firos} in
        n|no|N|No )
        ;;
        * )
            echo "installing firos, rcm_driver and robotics_msgs"
            install_rcm_component robotics_msgs.zip
            install_rcm_component rcm_driver.zip
            install_rcm_component firos.zip
        ;;
    esac
